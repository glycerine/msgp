...
func generate(f *parse.FileSet, mode gen.Method) (*bytes.Buffer, *bytes.Buffer, error) {
	...
	myImports := []string{"github.com/glycerine/msgp/msgp"}
	...
	if mode&gen.Test == gen.Test {
		...
		if mode&(gen.Encode|gen.Decode) != 0 {
			writeImportHeader(testbuf, "bytes", "github.com/glycerine/msgp/msgp", "testing")
		} else {
			writeImportHeader(testbuf, "github.com/glycerine/msgp/msgp", "testing")
		}
		testwr = testbuf
	}
