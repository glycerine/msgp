...
func TestMissingNilledOutWhenUnmarshallingOrDecodingNilIntoNestedStructs(t *testing.T) {
	...
	for passNo := 0; passNo < 2; passNo++ {
		...
		if !s.TopTime.IsZero() {
			panic("TopTime should have been zero-ed out")
		}

		if pTime != s.PtrTime {
			panic(fmt.Sprintf("PtrTime has changed on pass %v/'%s', and it should not have!. orig: %p, after decode: %p. ... &s.TopTime: %p", passNo, underTest, pTime, s.PtrTime, &s.TopTime))
		}

	} // end passNo loop
}

// the nil should be consumed iff its not a nbs.AlwaysNil nil.

// cases to consider:
//
// wire field is nil.
//   - a) target is nil pointer
